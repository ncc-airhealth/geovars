...
_chunk_oa_weight AS (
    ...
    LEFT JOIN output_area o ON ST_DWithin(c.geom, o.geom, b.radius)
)
, _stat_wide AS (
    SELECT
        c.id,
        ...
        c.radius,
        s.pop
            .multiply(c.weight).SUM().GREATEST(0)   AS POP,     -- 인구 수
        s.pop_m
            .multiply(c.weight).SUM().GREATEST(0)   AS POP_M,   -- 인구 수: 남성
        s.pop_f
            .multiply(c.weight).SUM().GREATEST(0)   AS POP_F,   -- 인구 수: 여성
        s.ho_gb_001
            .multiply(c.weight).SUM().GREATEST(0)   AS H_gb_1,  -- 주택유형별 주택 수: 다세대
        s.ho_gb_002
            .multiply(c.weight).SUM().GREATEST(0)   AS H_gb_2,  -- 주택유형별 주택 수: 단독주택
        s.ho_gb_003
            .multiply(c.weight).SUM().GREATEST(0)   AS H_gb_3,  -- 주택유형별 주택 수: 아파트
        s.ho_gb_004
            .multiply(c.weight).SUM().GREATEST(0)   AS H_gb_4,  -- 주택유형별 주택 수: 연립주택
        s.ho_gb_005.
            multiply(c.weight).SUM().GREATEST(0)    AS H_gb_5,  -- 주택유형별 주택 수: 영업용 건물 내 주택
        s.ho_gb_006
            .multiply(c.weight).SUM().GREATEST(0)   AS H_gb_6,  -- 주택유형별 주택 수: 주택 이외의 거처
        s.ga
            .multiply(c.weight).SUM().GREATEST(0)   AS GA,      -- 총 가구수
        CASE y.gv_year
            ...
            ELSE ho_yr_001
        END .multiply(c.weight).SUM().GREATEST(0)   AS H_yr_1,  -- 건축년도별 주택: ~1959
        CASE y.gv_year
            WHEN 2000 THEN ho_yr_004
            WHEN 2005 THEN ho_yr_004
            WHEN 2010 THEN ho_yr_004
            ELSE ho_yr_002
        END .multiply(c.weight).SUM().GREATEST(0)   AS H_yr_2,  -- 건축년도별 주택: 1960~1969
        CASE y.gv_year
            WHEN 2000 THEN ho_yr_005 + ho_yr_006
            WHEN 2005 THEN ho_yr_005 + ho_yr_006
            WHEN 2010 THEN ho_yr_005 + ho_yr_006
            ELSE ho_yr_003
        END .multiply(c.weight).SUM().GREATEST(0)   AS H_yr_3,  -- 건축년도별 주택: 1970~1979
        CASE y.gv_year
            WHEN 2000 THEN ho_yr_007
            WHEN 2005 THEN ho_yr_007
            WHEN 2010 THEN ho_yr_007
            ELSE ho_yr_004
        END .multiply(c.weight).SUM().GREATEST(0)   AS H_yr_4,   -- 건축년도별 주택: 1980~1989
        CASE y.gv_year
            ...
            ELSE ho_yr_005
        END .multiply(c.weight).SUM().GREATEST(0)   AS H_yr_5,   -- 건축년도별 주택: 1990~1999
        CASE y.gv_year
        ...
        END .multiply(c.weight).SUM().GREATEST(0)   AS H_yr_6,   -- 건축년도별 주택: 2000~2009
        CASE y.gv_year
            WHEN 2000 THEN 0
            WHEN 2005 THEN 0
            WHEN 2010 THEN 0
            ELSE ho_yr_011 + ho_yr_012 + ho_yr_013 + ho_yr_014 + ho_yr_015
        END .multiply(c.weight).SUM().GREATEST(0)   AS H_yr_7,   -- 건축년도별 주택: 2010~2019
        CASE y.gv_year
            WHEN 2000 THEN 0
            WHEN 2005 THEN 0
            WHEN 2010 THEN 0
            ELSE ho_yr_016
        END .multiply(c.weight).SUM().GREATEST(0)   AS H_yr_8,   -- 건축년도별 주택: 2020~
        CASE y.gv_year
            WHEN 2000 THEN cp_bnu_001 + cp_bnu_002
            WHEN 2005 THEN cp_bnu_001 + cp_bnu_002
            ELSE cp_bnu_001
        END .multiply(c.weight).SUM().GREATEST(0)   AS B_bnu_1,  -- 산업분류별 사업체수: 농업,임업,어업
        CASE y.gv_year
            WHEN 2000 THEN cp_bnu_003 + cp_bnu_004
            WHEN 2005 THEN cp_bnu_003 + cp_bnu_004
            ELSE cp_bnu_002 + cp_bnu_003
        END .multiply(c.weight).SUM().GREATEST(0)   AS B_bnu_2,  -- 산업분류별 사업체수: 광업 및 제조업
        CASE y.gv_year
            WHEN 2000 THEN cp_bnu_005
            WHEN 2005 THEN cp_bnu_005
            ELSE cp_bnu_004 + cp_bnu_005
        END .multiply(c.weight).SUM().GREATEST(0)   AS B_bnu_3,  -- 산업분류별 사업체수: 전기,가스,수도사업 등
        s.cp_bnu_006
            .multiply(c.weight).SUM().GREATEST(0)   AS B_bnu_4,  -- 산업분류별 사업체수: 건설업
        s.cp_bnu_007
            .multiply(c.weight).SUM().GREATEST(0)   AS B_bnu_5,  -- 산업분류별 사업체수: 도매 및 소매업
        CASE y.gv_year
            WHEN 2000 THEN cp_bnu_009
            WHEN 2005 THEN cp_bnu_009
            ELSE cp_bnu_008
        END .multiply(c.weight).SUM().GREATEST(0)   AS B_bnu_6,  -- 산업분류별 사업체수: 운수업
        CASE y.gv_year
        ...
        CASE y.gv_year
            ...
            ELSE cp_bem_001
        END .multiply(c.weight).SUM().GREATEST(0)   AS B_bem_1,  -- 산업분류별 종사자수: 농업,임업,어업
        CASE y.gv_year
            WHEN 2000 THEN cp_bem_003 + cp_bem_004
            WHEN 2005 THEN cp_bem_003 + cp_bem_004
            ELSE cp_bem_002 + cp_bem_003
        END .multiply(c.weight).SUM().GREATEST(0)   AS B_bem_2,  -- 산업분류별 종사자수: 광업 및 제조업
        CASE y.gv_year
            WHEN 2000 THEN cp_bem_005
            WHEN 2005 THEN cp_bem_005
            ELSE cp_bem_004 + cp_bem_005
        END .multiply(c.weight).SUM().GREATEST(0)   AS B_bem_3,  -- 산업분류별 종사자수: 전기,가스,수도사업 등
        s.cp_bem_006
            .multiply(c.weight).SUM().GREATEST(0)   AS B_bem_4,  -- 산업분류별 종사자수: 건설업
        s.cp_bem_007
            .multiply(c.weight).SUM().GREATEST(0)   AS B_bem_5,  -- 산업분류별 종사자수: 도매 및 소매업
        CASE y.gv_year
            WHEN 2000 THEN cp_bem_009
            WHEN 2005 THEN cp_bem_009
            ELSE cp_bem_008
        END .multiply(c.weight).SUM().GREATEST(0)   AS B_bem_6,  -- 산업분류별 종사자수: 운수업
        CASE y.gv_year
